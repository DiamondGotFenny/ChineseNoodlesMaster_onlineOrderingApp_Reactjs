  ...
  return (
    <Tab.Container defaultActiveKey='basic-info'>
      <Nav variant='tabs'>
        {tabData.map((item) => (
          <Nav.Item key={item.id}>
            <Nav.Link id={item.id} aria-controls={item.id} eventKey={item.id}>
              {item.label}
            </Nav.Link>
          </Nav.Item>
        ))}
      ...
      <Tab.Content>
        ...
        <Tab.Pane eventKey='activities' className='activities'>
          <ActivitiesContainer />
        </Tab.Pane>
        <Tab.Pane eventKey='edit-profile' className='edit-profile'>
          <EditProfile />
        </Tab.Pane>
      </Tab.Content>
    </Tab.Container>
