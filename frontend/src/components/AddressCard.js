...
const AddressCard = (props) => {
  ...
  const handleSetDefault = () => {
    if (authInfo.status === 'sucess' && userInfo.data) {
      const orginalDefault = userInfo.data.address.find(
        (ele) => ele.addressTitle === 'Default Address'
      );
      const targetAdrs = userInfo.data.address.find((ele) => ele.id === id);
      const updatedAddressList = userInfo.data.address.filter(
        (ele) => ele.addressTitle !== 'Default Address' && ele.id !== id
      );
      if (orginalDefault) {
  ...
  return (
    <Card>
      ...
      <Card.Body>
        ...
        <Card.Text>
          Phone Number: <span className='adrs-phone-num'>{phoneNumber}</span>
        </Card.Text>
