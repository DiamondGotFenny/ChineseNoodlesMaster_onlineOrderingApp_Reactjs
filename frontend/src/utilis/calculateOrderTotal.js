...
export const getOrderChargeInfo = (items, extraCharge) => {
  const subTotal = calculateOrderSubTotal(items);
  const discountVal = calDiscountVal(subTotal, extraCharge.discount);
  const vatVal = calVatVal(subTotal, extraCharge.vat);
  const deliveryFee = calDeliveryFee(subTotal, extraCharge.deliveringFee);
  ...
  return { subTotal, discountVal, vatVal, deliveryFee, totalCost };
};
