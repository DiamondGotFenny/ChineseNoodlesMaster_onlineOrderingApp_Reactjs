   ...
   const getFilteredList=(list,targetVal)=>{
        if (list.length>0) {
            const ordersBetweenArr= list.filter(item=>{
                ...
                const dateBetween= CalculateDateBetween(currentDate,item);
    ...
    return (
        <>
            ...
            <Form>
                <Form.Row>
                    <Col>
                        <Form.Control id="sortByTime" as="select" defaultValue={defaultBetweenVal} onChange={handleTimeValueChanges}>
            ...
            {list.length>0&&list.map(item=><OrderCard key={item.id} item={item} />)}
