...
const EditAddressModal = (props) => {
  ...
  const handleOnChange = (e, key, key2) => {
    const newObj = cloneDeep(adrsInfo);
  ...
  const handleEditSubmit = (e) => {
    ...
    if (authInfo.status === 'sucess' && userInfo.status === 'sucess') {
      /*here we have to update the whole userProfile in order to update address object 
            ...
            */
      if (item) {
        //if we are editing a existed address
        //we find its index in the address list array first, then we can update it in the array
        const index = userInfo.data.address.findIndex(
          (item) => item.id === adrsInfo.id
        );
        const updatedUserProfile = update(userInfo.data, {
          address: { [index]: { $set: adrsInfo } },
        });
        dispatch(
          updateUserInfoAction(authInfo, updatedUserProfile, userInfo.data.id)
        );
      } else {
  ...
  return (
    <Modal show={show} onHide={handleClose} id='editAdrsMod'>
      <Modal.Header closeButton>
        <Modal.Title className='editAdrs'>
        ...
        <Form>
          <Form.Group controlId='addressTitle'>
            ...
            <Form.Control
              ...
              onChange={(e) => handleOnChange(e, 'addressTitle')}
            />
          </Form.Group>
          <Form.Group controlId='addressReciever'>
            ...
            <Form.Control
              type='input'
              value={receiver}
              onChange={(e) => handleOnChange(e, 'receiver')}
            />
          </Form.Group>
          ...
          <Form.Group controlId='addressline1'>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                handleOnChange(e, 'addressLine1', 'addressDetail')
              }
            />
          </Form.Group>
          <Form.Group controlId='addressline2'>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                handleOnChange(e, 'addressLine2', 'addressDetail')
              }
            />
          </Form.Group>
          <Form.Group controlId='addressDistrict'>
            ...
            <Form.Control
              ...
              onChange={(e) => handleOnChange(e, 'district', 'addressDetail')}
            />
          </Form.Group>
